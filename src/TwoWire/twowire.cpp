...
TwoWire::TwoWire(){             // i2c interface
    ...
    /* default software I2C */
    twSetting.hw_interface = soft_I2C;
...
void TwoWire::close() {
  ...
  /* set to input becouse it's safer to RPi */
  pinMode(twSetting.twi_sda, INPUT);
  pinMode(twSetting.twi_scl, INPUT);
...
Wstatus TwoWire::i2c_write(uint8_t *buff, uint8_t length) {
...
Wstatus TwoWire::i2c_read(uint8_t *buff, uint8_t length) {
...
Wstatus TwCore::twi_init() {
  ...
  if (twSetting.Pullup)
  {
      if (twSetting.twi_sda != 2)
        pullUpDnControl(twSetting.twi_sda, PUD_UP);

      if (twSetting.twi_scl != 3)
        pullUpDnControl(twSetting.twi_scl, PUD_UP);
  }
